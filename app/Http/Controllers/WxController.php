            ...
            switch ($obj->MsgType) {
                case "event":
                    if ($obj->Event == "subscribe") {
                        ...
                        //掉接口
                        $user = json_decode($this->file_get_conterns($url), true);//跳方法 用get  方式调第三方类库
                        // $this->writeLog($fens);
                ...
                case "text":
                    if($obj->Content=="讲一个笑话"){
                            ...
                            $res=json_decode($this->file_get_conterns($url),true);// 调用的笑话结果  并转化为了数组
...
//                         file_put_contents("tianqishuju.txt",$this->file_get_conterns($url));
                            $shuju=json_decode($this->file_get_conterns($url),true);
